...
<div id="box_content">
	...
	if(isset($_POST['conce'])&& $_POST['conce'] == 'alteracao'){
		...
		$arquivo_velho_id = mysql_query("SELECT arquivo FROM concessionaria")
			  or die(mysql_error());
		$rows=mysql_fetch_array($arquivo_velho_id);
		$arquivo_velho = $rows[0];
		chdir("../uploads/concessionariaskit/");
		if(move_uploaded_file($_FILES['arquivo']['tmp_name'],$_FILES['arquivo']['name'])) {
		@unlink($arquivo_velho);
		mysql_query("DELETE FROM concessionaria WHERE arquivo = '$arquivo_velho'");
		$insert_arquivo_alterado=mysql_query("INSERT INTO concessionaria (arquivo) VALUES ('$arquivo_alterado')")
			or die(mysql_error());
